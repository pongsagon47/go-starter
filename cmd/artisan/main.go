...
import (
	...
	"flex-service/config"
	pkgDatabase "flex-service/pkg/database"
...
func toPascalCase(s string) string {
	...
	for i, word := range words {
		// Special handling for "id" and "uuid" at the end of words
		lowerWord := strings.ToLower(word)
		if lowerWord == "id" || lowerWord == "uuid" {
...
func getGormTag(field Field) string {
	...
	// Delete type:integer and type:bigint from tags if field is foreign key
	if field.IsForeignKey {
		for i, tag := range tags {
			if strings.Contains(tag, "type:integer") || strings.Contains(tag, "type:bigint") {
...
func getCreateTableTemplate(dbType string) string {
	...
	default:
		return createTableTemplate // Default to SQLite for compatibility
	}
}
...
func getAlterTableTemplate(dbType string) string {
	switch strings.ToLower(dbType) {
	case "sqlite":
		return alterTableTemplate
	case "mysql":
		return alterTableTemplateMySQL
	case "postgresql", "postgres":
		return alterTableTemplatePostgreSQL
	default:
		return alterTableTemplate // Default to SQLite for compatibility
	}
}
...
func getPrimaryKeyFields(data interface{}) string {
	...
	switch strategy {
	case "uuid":
		return `UUID      uuid.UUID ` + "`json:\"id\" gorm:\"type:varchar(36);index;unique;not null\"`"
	case "dual":
...
func getMigrationPrimaryKeyFields(data interface{}) string {
	...
	case "uuid":
		...
		default: // SQLite, MySQL
			return `UUID      uuid.UUID ` + "`gorm:\"type:varchar(36);primaryKey;not null;index\"`"
		}
	case "dual":
		switch strings.ToLower(dbType) {
		case "postgresql", "postgres":
